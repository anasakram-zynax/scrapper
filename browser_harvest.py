...
SLIDER_SETTLE_SEC = 15.0        # let DataDome finish rendering before auto-solving
_SOLVE_COOLDOWN_SEC = 2.0
_LAST_SOLVE_AT = 0.0
PROBE_FAIL_LIMIT = 5  # after this many failed probes, reload the page
...
def run_harvest_loop(
    ...
    *,
    ...
    stall = HarvestStallTracker()
    probe_fail_count = 0
    slider_seen_at: float | None = None
    ...
    while True:
        ...
        slider_present = is_datadome_slider_html(src)
        if slider_present and slider_seen_at is None:
            slider_seen_at = time.monotonic()
            if not quiet:
                print(
                    f">>> DataDome slider detected — waiting {SLIDER_SETTLE_SEC:g}s "
                    "for it to finish loading...\n",
                    flush=True,
                )
        elif not slider_present:
            slider_seen_at = None
        ...
        if tracker.update(cookie=cookie, challenged=challenged, src=src):
            ...
            if not pending and finalize_harvest(cookie, src):
                ...
                if not quiet:
                    print(
                        f">>> Page looks ready but HTTP session probe failed "
                        f"({probe_fail_count}/{PROBE_FAIL_LIMIT}) — "
                        "waiting for captcha to finish...\n",
                        flush=True,
                    )
                if probe_fail_count >= PROBE_FAIL_LIMIT:
                    if not quiet:
                        print(
                            f">>> Probe failed {PROBE_FAIL_LIMIT}x in a row — "
                            "ending this session and closing the window...\n",
                            flush=True,
                        )
                    raise HarvestConnectivityError("probe_failed_repeatedly")
        ...
        if can_solve and (challenged or tracker.awaiting_followup(cookie=cookie, src=src)):
            # DataDome can expose its iframe before the slider is interactive.
            # Do not send an early solve action; wait a full settling window.
            if slider_present and slider_seen_at is not None:
                if time.monotonic() - slider_seen_at < SLIDER_SETTLE_SEC:
